...
func TestRoleCreationRequiresRoleManagementMenu(t *testing.T) {
	...
	_, _, err := roles.SaveBasic(context.Background(), "2", &model.Role{Name: "越权创建"})
...
func TestRoleCreationRejectsUnknownActor(t *testing.T) {
	...
	_, _, err := roles.SaveBasic(context.Background(), "missing", &model.Role{Name: "无身份创建"})
